...
to be memorable. Your task has been made easy, as the encryption key
consists of three lower case characters. Using cipher.txt (right click
and 'Save Link/Target As...'), a file containing the encrypted ASCII
...
from utils import timer, ascii_sum
import itertools

KEYWORD = ' the '


with open('./data/059_xor_decryption.txt', 'r') as f:
    DATA = f.readlines()
    DATA = [int(l) for l in DATA[0].strip().split(',')]


def decrypt(key):
    x = []

    for c, k in zip(DATA, itertools.cycle(key)):
        x.append(chr(c ^ k))
...
def main():
    ...
    for a in range(low_bound, high_bound + 1):
        for b in range(low_bound, high_bound + 1):
            for c in range(low_bound, high_bound + 1):
                key = [a, b, c]

                decrypted = decrypt(key)
                if KEYWORD in decrypted:
                    print(decrypted)
                    print('Key: ' + ''.join(list(chr(c) for c in key)))
                    return ascii_sum(decrypted)
