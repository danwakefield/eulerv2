...
def main():
    not_checked = []
    tris = tri_numbers_gen()
    # Slight improvement in runtime ~20%. Skips 30 factorizations until one
    # hits 250 then start doing them all.
    while True:
        if len(not_checked) < 30:
            not_checked.append(next(tris))
        else:
            x = not_checked[-1]
            if len(factors(x)) > 250:
